...
import { Filter, Gamepad2, Home, ListChecks, Search } from "lucide-react";
...
export default async function MediaHome({ searchParams }: PageProps) {
  const query = await searchParams;
  const { items, total, page, pageSize } = await getItems(query);
  const serialized = items.map(serializeItem) as MediaItemView[];
    ...
    <main className="min-h-screen bg-[#09090b] text-zinc-100">
      <div className="border-b border-white/10 bg-zinc-950/90">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-5 lg:px-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-2xl font-semibold tracking-normal text-white">Luminary 媒体站</h1>
              <p className="text-sm text-zinc-400">影视、书籍与动画漫画档案</p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Link href="/games" className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-emerald-300/30 bg-emerald-400/10 px-4 text-sm font-medium text-emerald-100 transition hover:bg-emerald-400/15"><Gamepad2 size={16} />游戏站</Link>
          ...
          <form className="flex flex-col gap-3 md:flex-row" action="/media">
          ...
          <nav className="flex gap-2 overflow-x-auto pb-1">
            <Link href="/media" className={(activeType ? "bg-white/5 text-zinc-300" : "bg-amber-400 text-zinc-950") + " whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium"}>全部</Link>
            {mediaTypes.map((type) => (
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[260px_1fr] lg:px-6">
        <aside className="h-fit rounded-lg border border-white/10 bg-zinc-950/70 p-4">
          ...
          <form className="space-y-4" action="/media">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between text-sm text-zinc-400"><span>共 {total} 个媒体条目</span><Badge className="border-white/10 bg-white/5 text-zinc-300">第 {page} 页 / 每页 {pageSize}</Badge></div>
          {serialized.length ? <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6">{serialized.map((item: MediaItemView) => <ItemCard key={item.id} item={item} />)}</div> : <div className="rounded-lg border border-dashed border-white/10 bg-zinc-950/50 p-10 text-center text-zinc-400">暂无媒体条目，点击右上角新建。</div>}
        </section>
      </div>
    </main>
  );
